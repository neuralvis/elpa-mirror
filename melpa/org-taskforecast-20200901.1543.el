...
;; Package-Version: 20200901.1543
;; Package-Commit: c66afc3914acfd408e3fa0e68672c5babfe4db27
...
(defcustom org-taskforecast-list-task-link-formatters
  (list #'org-taskforecast-list-tlfmt-default-section-id
        ...
        #'org-taskforecast-list-tlfmt-tags)
...
;; TODO: when dropping support of org < 9.2, remove this function
;;       and use `org-get-tags' instead.
(defun org-taskforecast--get-tags (&optional local)
  "Tag strings of a heading at point.
If LOCAL is non-nil, this function returns local tags only."
  (with-no-warnings
    ;; since org 9.2, `org-get-tags' accepts "pos" and "local" parameter and
    ;; `org-get-tags-at' has been obsoleted.
...
(cl-defun org-taskforecast-timestamp-planned-date-p (timestamp date day-start &optional warningp (default-warning-days org-deadline-warning-days))
  "Non-nil if the TIMESTAMP is scheduled at DATE or earier.
If WARNINGP is non-nil, warning days is also considered.
DEFAULT-WARNING-DAYS is an integer as DEADLINE's default warning days.
If TIMESTAMP has warning section, DEFAULT-WARNING-DAYS is ignored.
DAY-START is an integer, see `org-taskforecast-day-start'."
  (cl-assert (natnump default-warning-days))
  (with-slots (ts-list) timestamp
    (-let* ((next-day-start
      ...
      (unless (memq warning-type '(all nil))
        ;; FIXME: "first" warning-type is not supported.
        ;;        I'm not sure what "first" warning-type means.
...
(defclass org-taskforecast--task ()
  ((id
   ...
   (local-tags
    :initarg :local-tags
    :reader org-taskforecast-task-local-tags
    :type list
    :documentation
    "Local tags.")
   (all-tags
    :initarg :all-tags
    :reader org-taskforecast-task-all-tags
    :type list
    :documentation
...
If the heading is not a task this function returns nil."
  ...
  (-when-let* ((element (org-taskforecast--heading-element-at-point))
    ...
    (let ((id (org-id-get-create))
          ...
          (local-tags (org-taskforecast--get-tags t))
          (all-tags (org-taskforecast--get-tags)))
...
DAY-START is an integer, see `org-taskforecast-day-start'."
  ...
  (-when-let (deadline (org-taskforecast-task-deadline task))
    (org-taskforecast-timestamp-planned-date-p deadline date day-start t)))
...
(defun org-taskforecast-tlink-local-tags (task-link)
  "Get local tags of TASK-LINK."
  (org-taskforecast-task-local-tags
   (org-taskforecast-tlink-task task-link)))

(defun org-taskforecast-tlink-all-tags (task-link)
  "Get all tags of TASK-LINK."
  (org-taskforecast-task-all-tags
   (org-taskforecast-tlink-task task-link)))
...
DAY-START is an integer like `org-taskforecast-day-start'."
  ;; Do not create FILE when FILE does not exist because the task list buffer
  ;; may be showing a task list for a past or future day.
  (when (or (file-exists-p file) (get-file-buffer file))
...
to nil."
  ...
  (let ((hooks '(org-clock-in-hook
                 ...
                 ;; FIXME: Support detecting file tags and parent tags update
...
`org-taskforecast-registration-filter-function'."
...
(defvar-local org-taskforecast--list-date nil
  "Date of task list.
If the value is nil, it means today.")

(defun org-taskforecast--list-get-date (day-start now)
  "Get date of task list of current buffer.
- DAY-START is an integer, see `org-taskforecast-day-start'
- NOW is an encoded time"
  (or org-taskforecast--list-date
      (org-taskforecast--date-of-time now day-start)))

(defun org-taskforecast--list-set-date (date)
  "Set date of task list of current buffer.
DATE is an encoded time."
  (setq org-taskforecast--list-date
        (org-taskforecast--time-as-date date)))

(defun org-taskforecast--list-reset-date ()
  "Reset date of task list of current buffer."
  (setq org-taskforecast--list-date nil))
...
This function is used for `org-taskforecast-list-header-formatters'."
  ;; TODO: define face
  (let ((face (if (org-taskforecast--time-equal-p
                   (org-taskforecast--date-of-time
...
(org-taskforecast--list-define-toggleable-tlfmt org-taskforecast-list-tlfmt-tags t
  "Format task's tags.
This function is used for `org-taskforecast-list-task-link-formatters'."
  (let ((all-tags (org-taskforecast-tlink-all-tags
                   org-taskforecast-list-info-task-link))
        (local-tags (org-taskforecast-tlink-local-tags
                     org-taskforecast-list-info-task-link)))
    (-some--> (concat (-some--> all-tags
                        (-difference it local-tags)
                        (s-join ":" it)
                        (s-wrap it ":" (if local-tags ":" "::")))
                      (-some--> local-tags
                        (s-join ":" it)
                        (s-wrap it ":" ":")))
      ;; TODO: define face
      (propertize it 'face 'org-tag)
      (concat
       (propertize " "
                   ;; "-1" is to prevent wrapping and truncating when
                   ;; emacs has no fringes.
                   'display `((space :align-to (- right ,(length it) 1))))
       it))))
...
(defclass org-taskforecast--transient-formatter-suffix (transient-suffix)
  ((switch :initarg :switch :type symbol))
  :description
  "Transient suffix to toggle status of a formatter.")

(cl-defmethod transient-format-description ((obj org-taskforecast--transient-formatter-suffix))
  "Show description of OBJ with its status.
The symbol's value of the switch slot of OBJ is used as the status."
  (when-let ((desc (oref obj description))
             (switch (oref obj switch)))
    (--> (if (functionp desc)
         ...
         (format "%s (%s)"
                 it
                 (if (buffer-local-value switch transient--original-buffer)
                     ;; TODO: define face
                     (propertize "on" 'face 'transient-argument)
                   (propertize "off" 'face 'transient-inactive-argument))))))

(transient-define-prefix org-taskforecast-list-toggle-formatter ()
  "Toggle formatter."
  ["Toggle formatter"
   [("d" "Default section"
     org-taskforecast-list-tlfmt-default-section-id-toggle
     :class org-taskforecast--transient-formatter-suffix
     :switch org-taskforecast-list-tlfmt-default-section-id-show)
    ("S" "Scheduled time"
     org-taskforecast-list-tlfmt-scheduled-time-toggle
     :class org-taskforecast--transient-formatter-suffix
     :switch org-taskforecast-list-tlfmt-scheduled-time-show)
    ("s" "Start time"
     org-taskforecast-list-tlfmt-start-toggle
     :class org-taskforecast--transient-formatter-suffix
     :switch org-taskforecast-list-tlfmt-start-show)
    ("e" "End time"
     org-taskforecast-list-tlfmt-end-toggle
     :class org-taskforecast--transient-formatter-suffix
     :switch org-taskforecast-list-tlfmt-end-show)]
   [("f" "Effort"
     org-taskforecast-list-tlfmt-effort-toggle
     :class org-taskforecast--transient-formatter-suffix
     :switch org-taskforecast-list-tlfmt-effort-show)
    ("c" "Clock"
     org-taskforecast-list-tlfmt-clock-toggle
     :class org-taskforecast--transient-formatter-suffix
     :switch org-taskforecast-list-tlfmt-clock-show)
    ("t" "Tags"
     org-taskforecast-list-tlfmt-tags-toggle
     :class org-taskforecast--transient-formatter-suffix
     :switch org-taskforecast-list-tlfmt-tags-show)]])

;;;;; Filter

;;;;;; Utility

(defvar org-taskforecast--list-filter-info-task-link nil
  "This variable is used to pass a task link to a filter function.
A filter function is called by `org-taskforecast--list-filter-accept-p'.
This value will be an instance of `org-taskforecast--tlink'.")

(defvar org-taskforecast--list-filter-info-today nil
  "This variable is used to pass the date of today to a filter function.
A filter function is called by `org-taskforecast--list-filter-accept-p'.
This value will be an encoded time.")

(defvar org-taskforecast--list-filter-function nil
  "A function to filter task links for displaying.")

(defun org-taskforecast--list-filter-set-filter (fn)
  "Set FN as a filter function for `org-taskforecast-list-mode'."
  (setq org-taskforecast--list-filter-function fn))

(defun org-taskforecast--list-filter-clear-filter ()
  "Clear filter for `org-taskforecast-list-mode'."
  (setq org-taskforecast--list-filter-function nil))

(defun org-taskforecast--list-filter-accept-p (task-link date day-start)
  "Non-nil means TASK-LINK is accepted by the filter function.
If `org-taskforecast--list-filter-function' is not a function, the
returned value is non-nil.
- DATE is an encoded time as a date of today
- DAY-START is an integer, see `org-taskforecast-day-start'"
  (if (functionp org-taskforecast--list-filter-function)
      (let ((org-taskforecast--list-filter-info-task-link task-link)
            (org-taskforecast--list-filter-info-today date)
            (org-taskforecast-day-start day-start))
        (funcall org-taskforecast--list-filter-function))
    t))
...
(defun org-taskforecast-list-filter-by-todo-state (state now)
  "Filter task lists by todo state STATE.
- STATE is a symbol, `todo' or `done'
- NOW is an encoded time as the current time"
  (interactive
   (let* ((input (completing-read "Filter by todo state (TODO/DONE): "
                                  '("TODO" "DONE") nil t))
          (state (alist-get input '(("TODO" . todo) ("DONE" . done))
                            nil nil #'equal))
          (now (current-time)))
     (cl-assert state)
     (list state now)))
  (org-taskforecast--list-filter-set-filter
   (lambda ()
     (eq state
         (org-taskforecast-entry-todo-state-for-today
          org-taskforecast--list-filter-info-task-link
          org-taskforecast--list-filter-info-today
          org-taskforecast-day-start))))
  (org-taskforecast--list-refresh now))

;;;;;;; By effort

(defun org-taskforecast--list-filter-read-effort (prompt)
  "Read effort string, prompting with PROMPT.
The returned value is an effort time in seconds."
  (let* ((input (read-string prompt))
         (effort (org-taskforecast--effort-to-second input)))
    (if effort effort
      (error "Invalid effort string, %s" input))))

;; Interactive form seems that it cannot reference a symbol's value
;; via lexical binding.  So I use macro instead of `defalias'.
(defmacro org-taskforecast--list-filter-define-filter-by-effort (name comparator prompt)
  "Define effort filter NAME with COMPARATOR and prompting with PROMPT."
  (declare (indent 1))
  `(defun ,name (effort now)
     ,(format "Filter task lists by effort with `%s'.
- EFFORT is an effort time as second
- NOW is an encoded time as the current time"
              comparator)
     (interactive
      (let* ((now (current-time))
             (effort (org-taskforecast--list-filter-read-effort ,prompt)))
        (list effort now)))
     (org-taskforecast--list-filter-set-filter
      (lambda ()
        (-when-let (tle (org-taskforecast-entry-effective-effort
                         org-taskforecast--list-filter-info-task-link
                         org-taskforecast--list-filter-info-today
                         org-taskforecast-day-start))
          (funcall #',comparator tle effort))))
     (org-taskforecast--list-refresh now)))

(org-taskforecast--list-filter-define-filter-by-effort
    org-taskforecast-list-filter-by-effort<=
  <= "Filter by effort less than or equals to: ")

(org-taskforecast--list-filter-define-filter-by-effort
    org-taskforecast-list-filter-by-effort>=
  >= "Filter by effort greater than or equals to: ")

(org-taskforecast--list-filter-define-filter-by-effort
    org-taskforecast-list-filter-by-effort=
  = "Filter by effort equals to: ")

(transient-define-prefix org-taskforecast-list-filter-by-effort ()
  "Filter task links by effort."
  ["Filter by effort"
   ("=" "Equals to" org-taskforecast-list-filter-by-effort=)
   ("<" "Less than or equals to" org-taskforecast-list-filter-by-effort<=)
   (">" "Greater than or equals to" org-taskforecast-list-filter-by-effort>=)])

;;;;;;; By tag

(defun org-taskforecast--list-filter-get-tags (file)
  "Get tags from daily task list file FILE."
  (--> (org-taskforecast--get-task-links file)
       (-map #'org-taskforecast-tlink-all-tags it)
       (apply #'append it)
       (-uniq it)))

(defun org-taskforecast-list-filter-by-tag (tag now)
  "Filter task lists by TAG.
NOW is an encoded time as the current time."
  (interactive
   (let* ((day-start org-taskforecast-day-start)
          (now (current-time))
          (date (org-taskforecast--list-get-date day-start now))
          (file (org-taskforecast-get-dailylist-file date))
          (tags (org-taskforecast--list-filter-get-tags file))
          (tag (completing-read "Filter by tag: " tags nil t)))
     (list tag now)))
  (org-taskforecast--list-filter-set-filter
   (lambda ()
     (member tag
             (org-taskforecast-tlink-all-tags
              org-taskforecast--list-filter-info-task-link))))
  (org-taskforecast--list-refresh now))

;;;;;;; By repeated

(defun org-taskforecast--list-filter-by-repeated (accept now)
  "Filter task links by whether repeated.
If ACCEPT is non-nil, the filter accepts only repeated task links.
If not, the filter rejects repeated task links.
NOW is an encoded time as the current time."
  (org-taskforecast--list-filter-set-filter
   (lambda ()
     (--> org-taskforecast--list-filter-info-task-link
          (org-taskforecast-tlink-task it)
          (org-taskforecast-task-repeat-p it)
          (if accept it (not it)))))
  (org-taskforecast--list-refresh now))

(defun org-taskforecast-list-filter-by-repeated-accept (now)
  "Filter task links with accepting only repeated ones.
NOW is an encoded time as the current time."
  (interactive (list (current-time)))
  (org-taskforecast--list-filter-by-repeated t now))

(defun org-taskforecast-list-filter-by-repeated-reject (now)
  "Filter task links with rejecting repeated ones.
NOW is an encoded time as the current time."
  (interactive (list (current-time)))
  (org-taskforecast--list-filter-by-repeated nil now))

(transient-define-prefix org-taskforecast-list-filter-by-repeated ()
  ["Filter by repeated"
   ("+" "Accept only repeated"
    org-taskforecast-list-filter-by-repeated-accept)
   ("-" "Reject repeated"
    org-taskforecast-list-filter-by-repeated-reject)])

;;;;;;; General commands

(transient-define-prefix org-taskforecast-list-filter ()
  "Filter task links."
  ["Filter by"
   ("t" "TODO state" org-taskforecast-list-filter-by-todo-state)
   ("r" "Repeated" org-taskforecast-list-filter-by-repeated)
   ("e" "Effort" org-taskforecast-list-filter-by-effort)
   (":" "Tag" org-taskforecast-list-filter-by-tag)]
  [("/" "Clear filter" org-taskforecast-list-filter-clear)])

(defun org-taskforecast-list-filter-clear (now)
  "Clear current filter.
NOW is an encoded time as the current time."
  (interactive (list (current-time)))
  (org-taskforecast--list-filter-clear-filter)
  (org-taskforecast--list-refresh now))
         ...
         (--map
          (if (org-taskforecast-entry-is-task-link it)
              (let* ((todo-type
                ...
                (prog1
                    (when (org-taskforecast--list-filter-accept-p
                           it today day-start)
...
- NOW is an encoded time"
  (let ((entries (org-taskforecast--get-entries
                  ...
                  day-start)))
    (insert
     (org-taskforecast--list-create-header entries today day-start now)
...
(defvar org-taskforecast-list-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd ".") #'org-taskforecast-list-goto-today)
    (define-key map (kbd "j") #'org-taskforecast-list-goto-date)
    (define-key map (kbd "f") #'org-taskforecast-list-forward-date)
    (define-key map (kbd "b") #'org-taskforecast-list-backward-date)
    ...
    (define-key map (kbd "v") #'org-taskforecast-list-toggle-formatter)
    (define-key map (kbd "/") #'org-taskforecast-list-filter)
    map)
...
(defun org-taskforecast--create-list-buffer (day-start now)
        ...
        (with-current-buffer (get-buffer-create
          ...
          (save-excursion
            (let ((inhibit-read-only t)
                  (today (org-taskforecast--list-get-date day-start now)))
              (org-taskforecast--insert-task-list today day-start now)))
          (current-buffer)))))

;;;###autoload
(defun org-taskforecast-list (day-start now)
  "Show the buffer of `org-taskforecast-list-mode'.
- DAY-START is an integer, see `org-taskforecast-day-start'
- NOW is an encoded time"
  (interactive (list org-taskforecast-day-start (current-time)))
  (switch-to-buffer
   (org-taskforecast--create-list-buffer day-start now)))
...
(cl-defun org-taskforecast--list-refresh (now &optional (method 'id))
  "Refresh `org-taskforecast-list-mode' buffer.
NOW is an encoded time.
METHOD indicates how to keep/move the cursor line, its value can be
one of the symbols below:
- `id': keep the cursor line at the entry which was at the cursor line
  before refreshing
- `line': keep the line number of the cursor line
- `top': keep the cursor line at the top of the buffer"
    ...
    (with-current-buffer buffer
      (org-taskforecast--save-window-start buffer
        (let ((inhibit-read-only t)
              ...
              (today (org-taskforecast--list-get-date
                      org-taskforecast-day-start now)))
          (erase-buffer)
          ...
          (cond ((eq method 'top)
                 (goto-char (point-min)))
                ((and (eq method 'id) current-entry)
                 (goto-char (point-min))
                 (-some--> current-entry
                   (save-excursion
                     (cl-loop until (eobp)
                ...
                ((and (eq method 'id) eobp)
                 (goto-char (point-max)))
                ((or (and (eq method 'id) (not current-entry))
                     (eq method 'line))
                 ;; `goto-line' is for interactive use only
                 (goto-char (point-min))
                 (forward-line (1- current-line)))
                (t
                 (error "Unexpected state to refresh"))))))))
...
(defun org-taskforecast-list-goto-today (now)
  "Change date of task list to today.
NOW is an encoded time."
  (interactive (list (current-time)))
  (org-taskforecast--list-reset-date)
  (org-taskforecast--list-refresh now 'top))

(defun org-taskforecast-list-goto-date (date now)
  "Change date of task list to DATE.
- DATE is an encoded time
- NOW is an encoded time"
  (interactive
   (let ((now (current-time)))
     (list
      (--> (org-read-date
            nil t nil nil
            (org-taskforecast--list-get-date org-taskforecast-day-start now)))
      now)))
  (org-taskforecast--list-set-date date)
  (org-taskforecast--list-refresh now 'top))

(defun org-taskforecast-list-forward-date (n day-start now)
  "Forward N days of the date of task list.
- DAY-START is an integer, see `org-taskforecast-day-start'
- NOW is an encoded time"
  (interactive
   (list (prefix-numeric-value current-prefix-arg)
         org-taskforecast-day-start
         (current-time)))
  (--> (org-taskforecast--list-get-date day-start now)
       (decode-time it)
       (prog1 it
         (cl-callf + (org-taskforecast--decoded-time-day it) n))
       (apply #'org-taskforecast--encode-time it)
       (org-taskforecast--list-set-date it))
  (org-taskforecast--list-refresh now 'top))

(defun org-taskforecast-list-backward-date (n day-start now)
  "Backward N days of the date of task list.
- DAY-START is an integer, see `org-taskforecast-day-start'
- NOW is an encoded time"
  (interactive
   (list (prefix-numeric-value current-prefix-arg)
         org-taskforecast-day-start
         (current-time)))
  (org-taskforecast-list-forward-date (- n) day-start now))
...
NOW is an encoded time."
  ...
  (org-taskforecast--list-with-task-link task-link
    (org-taskforecast--at-id (org-taskforecast-tlink-task-id task-link)
      (org-todo))
    ;; Keep the line number of the cursor line because this command
    ;; may be used to change the state of some entries already done to
    ;; done.  If it kept entry ID not the line number, the cursor would be
    ;; moved to the head of todo entries with the entry and user would
    ;; need to move the cursor to the next entry to change its state to
    ;; done for each time.
    (org-taskforecast--list-refresh now 'line)))
...
NOW is an encoded time."
  ...
  (org-taskforecast--list-with-entry entry
    (cl-assert (/= arg 0))
    (let ((move-up-p (cl-plusp arg))
          (times (abs arg))
          target)
      (save-excursion
        (cl-loop repeat times
                 do (forward-line (if move-up-p -1 1))
                 for e = (org-taskforecast--list-get-entry-at-point)
                 while e
                 do (setq target e)))
      (unless target
        (user-error "Entry cannot be moved"))
      (let ((current-heading
             (org-taskforecast--cut-heading-by-id
              (org-taskforecast-entry-id entry))))
        (org-taskforecast--at-id (org-taskforecast-entry-id target)
          (unless move-up-p
            (goto-char (org-end-of-subtree t t)))
          (insert current-heading))))
    (org-taskforecast--list-refresh now)))
...
- NOW is an encoded time as the current time"
  ...
  (let* ((task-link-heading
    ...
    ;; Keep the line number of the cursor line because this command
    ;; may be used to move some entries which are placed on unexpected
    ;; sections.  If it kept entry ID not the line number, the cursor would
    ;; be moved to the expected section with the entry and user would need to
    ;; move the cursor to the next entry to move for each time.
    (org-taskforecast--list-refresh now 'line)))
...
- DAY-START is an integer like `org-taskforecast-day-start'"
  (interactive
   (let ((now (current-time))
         (day-start org-taskforecast-day-start))
     (list (org-taskforecast--list-get-date day-start now) day-start)))
  (let (p)
    (save-excursion
      (goto-char (point-min))
