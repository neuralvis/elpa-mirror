...
(defcustom undo-fu-session-incompatible-files nil
  "Regexps for matching file names to ignore saving/recovering undo history."
  :group 'undo-fu-session
  :type 'string)
...
(defun undo-fu-session--recover-buffer-p (buffer)
  ...
  (let
    ...
    (cond
      ((null filename)
        nil)
      ((and test-files (string-match test-files filename))
        nil)
